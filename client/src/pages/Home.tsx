...
type Cue = { start: number; end: number; text: string; translation?: string };
...
export default function Home() {
  ...
  async function listenTranslateCurrent() {
    ...
    worker.onmessage = (event: MessageEvent<{ type: string; result?: unknown; message?: string }>) => {
      ...
      if (data.type === "translation") {
        const result = data.result as Array<{ translation?: string; translation_text?: string }> | { translation?: string; translation_text?: string } | undefined;
        const first = Array.isArray(result) ? result[0] : result;
        translated[translateIndex].translation = (first?.translation_text ?? first?.translation ?? "").trim();
        translateIndex += 1;
    ...
    <div className="min-h-screen bg-[#0a0b0b] text-[#f4f1ea] selection:bg-[#c9ff65] selection:text-[#0a0b0b]">
      ...
      <main className="mx-auto grid min-h-[calc(100vh-72px)] max-w-[1440px] grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="relative flex min-h-[calc(100vh-72px)] flex-col border-r border-white/[0.08] px-5 py-7 lg:px-10 lg:py-10">
          ...
          {!current ? (
            ...
            <div className="relative flex flex-1 flex-col overflow-hidden rounded-[28px] border border-white/[0.1] bg-[#101212] shadow-2xl">
              <div className="relative flex min-h-[390px] flex-1 items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_50%_30%,rgba(201,255,101,0.08),transparent_38%),#111313]">
                ...
                {activeCue && <div className="pointer-events-none absolute bottom-7 left-1/2 max-w-[88%] -translate-x-1/2 rounded-xl bg-black/80 px-5 py-3 text-center shadow-lg backdrop-blur-md" style={{ transform: `translateX(-50%) translateY(${-captionOffset}px)` }}><div className="font-display text-lg" style={{ fontSize: `${captionSize}rem` }}>{activeCue.text}</div>{translationEnabled && activeCue.translation && <div className="mt-1 border-t border-white/15 pt-1 text-sm text-[#dfffaa]">{activeCue.translation}</div>}</div>}
          ...
          <div className="relative mt-5 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3 text-[11px] text-white/35"><span className="flex items-center gap-1.5"><Check size={13} className="text-[#c9ff65]" /> 瀏覽器本機處理</span><span className="h-1 w-1 rounded-full bg-white/20" /><span>本機播放不自動上傳</span></div><div className="flex flex-wrap gap-2"><button onClick={() => void transcribeCurrent()} disabled={!current || transcribing} className="secondary-button disabled:cursor-wait disabled:opacity-50"><Mic2 size={15} /> {transcribing ? "Whisper 轉錄中…" : "Whisper 自動轉錄"}</button><button onClick={() => void listenTranslateCurrent()} disabled={!current || transcribing || translating} className="secondary-button disabled:cursor-wait disabled:opacity-50"><Languages size={15} /> {translating ? "聽譯處理中…" : "一鍵聽譯（英→中）"}</button><button onClick={() => subtitleInput.current?.click()} className="secondary-button"><Subtitles size={15} /> 載入字幕</button><button onClick={() => current && void uploadToLibrary(current.file)} disabled={!current || !!uploadingName} className="secondary-button disabled:opacity-50"><FolderOpen size={15} /> {uploadingName ? "保存中…" : "保存至媒體庫"}</button><button onClick={() => mediaInput.current?.click()} className="secondary-button"><FolderOpen size={15} /> 新增媒體</button></div></div>
...
function SessionIntro({ onOpen播放清單, onOpen字幕 }: { onOpen播放清單: () => void; onOpen字幕: () => void }) {
  return <div className="flex h-full min-h-[calc(100vh-72px)] flex-col justify-between p-6"><div><p className="eyebrow">工作階段說明</p><div className="mt-8 space-y-7"><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Headphones size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">本機播放</span></div><p className="text-xs leading-6 text-white/38">拖放檔案後，瀏覽器會建立暫時的本機網址；原始媒體不會自動離開這台裝置。需要跨裝置使用時，再手動保存到你的私人媒體庫。</p></div><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Languages size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">加入字幕</span></div><p className="text-xs leading-6 text-white/38">載入 SRT 或 WebVTT 字幕，再從字幕面板調整大小與位置。</p></div><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Mic2 size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">離線翻譯</span></div><p className="text-xs leading-6 text-white/38">裝置端模型可選擇使用。連線時下載一次，之後即可在支援的瀏覽器中不經雲端處理媒體。</p></div></div></div><div className="space-y-2"><button onClick={onOpen播放清單} className="flex w-full items-center justify-between rounded-xl border border-white/10 px-4 py-3 text-left text-xs text-white/55 transition hover:border-[#c9ff65]/30 hover:text-white"><span className="flex items-center gap-2"><ListMusic size={15} /> 開啟播放清單</span><ChevronDown className="-rotate-90" size={14} /></button><button onClick={onOpen字幕} className="flex w-full items-center justify-between rounded-xl border border-white/10 px-4 py-3 text-left text-xs text-white/55 transition hover:border-[#c9ff65]/30 hover:text-white"><span className="flex items-center gap-2"><Subtitles size={15} /> 設定字幕</span><ChevronDown className="-rotate-90" size={14} /></button></div></div>;
}
