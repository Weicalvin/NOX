...
function SessionIntro({ onOpenQueue, onOpenCaptions }: { onOpenQueue: () => void; onOpenCaptions: () => void }) {
  return <div className="flex h-full min-h-[calc(100vh-72px)] flex-col justify-between p-6"><div><p className="eyebrow">工作階段說明</p><div className="mt-8 space-y-7"><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Headphones size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">Play locally</span></div><p className="text-xs leading-6 text-white/38">Drop a file and your browser creates a temporary local URL. The original media never needs to leave this device.</p></div><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Languages size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">Add captions</span></div><p className="text-xs leading-6 text-white/38">Load an SRT or WebVTT track, then tune its size and position from the caption panel.</p></div><div><div className="mb-3 flex items-center gap-2 text-[#c9ff65]"><Mic2 size={16} /><span className="text-xs font-medium uppercase tracking-[0.12em]">Translate offline</span></div><p className="text-xs leading-6 text-white/38">On-device models are optional. Download once when online, then process supported media without cloud calls.</p></div></div></div><div className="space-y-2"><button onClick={onOpenQueue} className="flex w-full items-center justify-between rounded-xl border border-white/10 px-4 py-3 text-left text-xs text-white/55 transition hover:border-[#c9ff65]/30 hover:text-white"><span className="flex items-center gap-2"><ListMusic size={15} /> Open playback queue</span><ChevronDown className="-rotate-90" size={14} /></button><button onClick={onOpenCaptions} className="flex w-full items-center justify-between rounded-xl border border-white/10 px-4 py-3 text-left text-xs text-white/55 transition hover:border-[#c9ff65]/30 hover:text-white"><span className="flex items-center gap-2"><Subtitles size={15} /> Configure captions</span><ChevronDown className="-rotate-90" size={14} /></button></div></div>;
}

function ModelLibrary() {
  const models = [
    { name: "Whisper tiny", type: "語音轉文字", size: "約 75 MB", note: "適合快速測試與較弱裝置", url: "https://huggingface.co/Xenova/whisper-tiny" },
    { name: "Whisper base", type: "語音轉文字", size: "約 140 MB", note: "較佳辨識品質，建議桌面瀏覽器", url: "https://huggingface.co/Xenova/whisper-base" },
    { name: "NLLB-200 distilled 600M", type: "多語翻譯", size: "約 1.2 GB", note: "支援多語言，下載時間較長", url: "https://huggingface.co/Xenova/nllb-200-distilled-600M" },
  ];
  return <div className="flex-1 overflow-auto p-6"><div className="rounded-2xl border border-[#c9ff65]/20 bg-[#c9ff65]/[0.06] p-5"><div className="flex gap-3"><Sparkles size={18} className="mt-0.5 shrink-0 text-[#c9ff65]" /><div><p className="text-xs font-medium text-white/80">模型只會儲存在你的瀏覽器</p><p className="mt-2 text-[11px] leading-5 text-white/45">NOX 不代管模型檔案。請從官方 Hugging Face 頁面查看授權與下載內容；首次使用可能需要較長時間與足夠儲存空間。</p></div></div></div><div className="mt-6 space-y-3">{models.map((model) => <div key={model.name} className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-white/80">{model.name}</p><p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-[#c9ff65]">{model.type} · {model.size}</p></div><a href={model.url} target="_blank" rel="noreferrer" className="shrink-0 rounded-lg bg-[#c9ff65] px-3 py-2 text-[10px] font-bold text-[#0a0b0b] transition hover:bg-white">開啟下載頁</a></div><p className="mt-3 text-[11px] leading-5 text-white/35">{model.note}</p></div>)}</div><p className="mt-6 text-[10px] leading-5 text-white/25">提示：目前網站已提供官方模型下載入口；模型自動快取與 Whisper / 翻譯執行管線將在下一個功能階段接入。</p></div>;
}
