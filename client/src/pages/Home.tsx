    ...
    <div className="min-h-screen bg-[#0a0b0b] text-[#f4f1ea] selection:bg-[#c9ff65] selection:text-[#0a0b0b]">
      ...
      <main className="mx-auto grid min-h-[calc(100vh-72px)] max-w-[1440px] grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="relative flex min-h-[calc(100vh-72px)] flex-col border-r border-white/[0.08] px-5 py-7 lg:px-10 lg:py-10">
          ...
          <div className="relative mt-5 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3 text-[11px] text-white/35"><span className="flex items-center gap-1.5"><Check size={13} className="text-[#c9ff65]" /> 瀏覽器本機處理</span><span className="h-1 w-1 rounded-full bg-white/20" /><span>本機播放不自動上傳</span></div><div className="flex flex-wrap gap-2"><button onClick={() => void transcribeCurrent()} disabled={!current || transcribing} className="secondary-button disabled:cursor-wait disabled:opacity-50"><Mic2 size={15} /> {transcribing ? "Whisper 轉錄中…" : "Whisper 自動轉錄"}</button><button onClick={() => subtitleInput.current?.click()} className="secondary-button"><Subtitles size={15} /> 載入字幕</button><button onClick={() => current && void uploadToLibrary(current.file)} disabled={!current || !!uploadingName} className="secondary-button disabled:opacity-50"><FolderOpen size={15} /> {uploadingName ? "保存中…" : "保存至媒體庫"}</button><button onClick={() => mediaInput.current?.click()} className="secondary-button"><FolderOpen size={15} /> 新增媒體</button></div></div>
